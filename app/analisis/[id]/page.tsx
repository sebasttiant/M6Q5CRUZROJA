...
export default async function AnalysisDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const analysis = await prisma.analysis.findUnique({ where: { id }, include: { categories: { include: { subcauses: true }, orderBy: { category: "asc" } }, mainCauses: { orderBy: { position: "asc" } } } });
  ...
  return <AppShell email={user.email}>
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
      ...
      <section className="panel"><h2 className="detail-title">Cinco porqués</h2><div className="space-y-5">{analysis.mainCauses.map((cause) => <article className="cause-card" key={cause.id}><h3>Causa {cause.position}: {cause.cause}</h3><p className="mt-1 text-sm text-muted">Subcausa: {cause.subcause}</p><ol className="mt-4 grid gap-2 md:grid-cols-5">{[cause.why1, cause.why2, cause.why3, cause.why4, cause.why5].map((why, index) => <li className="why-card" key={index}><span>{index + 1}</span>{why}</li>)}</ol></article>)}</div></section></div>
