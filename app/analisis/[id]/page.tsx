...
export default async function AnalysisDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const analysis = await prisma.analysis.findFirst({ where: { id, ...analysisScope(user) }, include: { categories: { include: { subcauses: true } }, mainCauses: { orderBy: { position: "asc" } } } });
  if (!analysis) notFound();
  // Prisma cannot order by our institutional sequence, so the six blocks are arranged here.
  const orderedCategories = [...analysis.categories].sort(
    (left, right) => CATEGORIES.findIndex(({ key }) => key === left.category) - CATEGORIES.findIndex(({ key }) => key === right.category),
  );
  const statusAction = updateAnalysisStatus.bind(null, analysis.id);
  return <AppShell email={user.email} role={user.role}>
    <PageHeader eyebrow={analysis.code} title={analysis.finding} description={`${analysis.process} · ${analysis.eventDate.toLocaleDateString("es-CO", { timeZone: "UTC" })}`} action={<div className="flex flex-wrap items-center gap-2"><a className="button button-primary" href={`/api/export/pdf?id=${analysis.id}`}><FileDown size={17} />Descargar PDF</a><form action={statusAction} className="flex gap-2"><select name="status" defaultValue={analysis.status}>{Object.entries(STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><button className="button button-secondary">Actualizar</button></form></div>} />
    <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
      <div className="space-y-6"><section className="panel"><h2 className="detail-title">Evaluación 6M</h2><div className="grid gap-4 md:grid-cols-2">{orderedCategories.map((category) => <article className="category-card" key={category.id}><div className="flex justify-between gap-2"><h3 className="flex items-center gap-2"><span className="m-badge">{CATEGORIES.find(({ key }) => key === category.category)?.m}</span>{CATEGORIES.find(({ key }) => key === category.category)?.label}</h3><span className="valuation">{category.valuation}</span></div>{category.subcauses.length ? <ul className="mt-3 space-y-2">{category.subcauses.map((subcause) => <li className="flex justify-between gap-3 text-sm" key={subcause.id}><span>{subcause.description}</span><strong>Impacto {subcause.impact}</strong></li>)}</ul> : <p className="mt-3 text-sm text-muted">Sin subcausas registradas.</p>}</article>)}</div></section>
      <section className="panel"><h2 className="detail-title">Cinco porqués</h2><div className="space-y-5">{analysis.mainCauses.map((cause) => <article className="cause-card" key={cause.id}><h3>Causa {cause.position}: {cause.cause}</h3><p className="mt-1 text-sm text-muted">Subcausa: {cause.subcause}</p><ol className="mt-4 grid gap-2 md:grid-cols-3">{[cause.why1, cause.why2, cause.why3].map((why, index) => <li className="why-card" key={index}><span>{index + 1}</span>{why}</li>)}</ol></article>)}</div></section></div>
