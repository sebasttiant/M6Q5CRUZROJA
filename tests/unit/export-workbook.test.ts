...
const analysis: ExportAnalysis = {
  ...
  mainCauses: [{ position: 1, cause: "Método", subcause: "Procedimiento ambiguo", why1: "Respuesta 1", why2: "Respuesta 2", why3: "Respuesta 3" }],
};

describe("analysis Excel export", () => {
  it("contains subcauses, valuation, causes, three whys and root cause", async () => {
    ...
    expect(valueFor("Causa principal 1")).toBe("Método");
    expect(valueFor("Subcausa asociada 1")).toBe("Procedimiento ambiguo");
    for (let why = 1; why <= 3; why += 1) expect(valueFor(`Por qué ${why} — causa 1`)).toBe(`Respuesta ${why}`);
    expect(valueFor("Causa raíz")).toBe("Control insuficiente");
    expect(headers).not.toContain("Por qué 4 — causa 1");
  });
});
